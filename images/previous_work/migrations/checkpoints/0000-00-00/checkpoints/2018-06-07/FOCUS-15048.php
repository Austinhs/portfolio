...
$es_fields       = [
			...
			SELECT parent_id AS staff_id,
			       LTRIM(
			          RTRIM(
			             CONCAT(
			                COALESCE(address1, ''),
			                CASE WHEN address1 IS NOT NULL AND address2 IS NOT NULL THEN ', ' ELSE '' END,
			                COALESCE(address2, ''),
			                CASE WHEN address1 IS NOT NULL OR address2 IS NOT NULL AND city IS NOT NULL THEN ', ' ELSE '' END,
			                COALESCE(city, ''),
			                ', ',
			                COALESCE(state, ''),
			                ' ',
			                RIGHT(CONCAT('00000', zipcode), 5),
			                CASE WHEN plus4 IS NOT NULL THEN '-' ELSE '' END,
			                CASE WHEN plus4 IS NOT NULL THEN RIGHT(CONCAT('0000', plus4), 4) ELSE '' END
			             )
			          )
			       ) AS value
			FROM   gl_address
	...
	[
		...
		'type'           => 'computed_table',
		"computed_query" => "
			SELECT   c.parent_id AS staff_id, ct.title AS \"Type\", c.cont_data AS \"Contact Info\"
			FROM     gl_contact c
			JOIN     gl_contact_types ct ON ct.id = c.cont_type
